...
export function RecommendationsList({ data }) {
  const allRecommendations = data.flatMap((item) =>
      ...
      {allRecommendations.map((recommendation, index) => (
        <Card key={index}>
          ...
          <CardContent>
            ...
            <p className="mb-4 bg-gray-100 p-2 rounded">{recommendation.for_response}</p>
            <p className="font-semibold mb-2">Suggestion:</p>
            <p className="bg-blue-50 p-2 rounded">{recommendation.suggestion}</p>
          </CardContent>
        </Card>
      ))}
    </div>
