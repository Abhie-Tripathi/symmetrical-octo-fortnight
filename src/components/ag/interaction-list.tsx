        ...
        <Card key={interaction._id} className="overflow-hidden">
          <CardHeader className="bg-gray-100">
            <CardTitle className="flex justify-between items-center">
              ...
              <Badge
                className={getConfidenceColor(interaction.overall_confidence)}
              >
          ...
          <CardContent className="pt-4">
            <ul className="space-y-4">
              {interaction.confidence_scores.map(
                (score: any, index: number) => (
                  <li key={index} className="border-b pb-4 last:border-b-0">
                    <p className="font-semibold mb-1">
                      Q: {score.client_question}
                    </p>
                    <p className="mb-1">A: {score.bot_response}</p>
                    <Badge
                      className={getConfidenceColor(score.confidence_score)}
                    >
